...
const useHealthData = (date: Date) => {
  ...
  useEffect(() => {
    const checkAvailability = async () => {
      if (Platform.OS === 'ios') {
        await checkIOSAvailability();
      } else if (Platform.OS === 'android') {
        await checkAndroidAvailability();
      }
    };
  ...
  const fetchIOSHealthData = async () => {
    try {
      ...
      const stepsResult = await AppleHealthKit.getStepCount(options);
      setSteps(stepsResult.value);

      const flightsResult = await AppleHealthKit.getFlightsClimbed(options);
      setFlights(flightsResult.value);

      const distanceResult = await AppleHealthKit.getDistanceWalkingRunning(options);
      setDistance(distanceResult.value);
    } catch (error) {
